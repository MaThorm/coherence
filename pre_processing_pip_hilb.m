function [hilbert_angles,inst_freq,filt_inst_freq,instchange,summary] = pre_processing_pip_hilb(trials,filttype,framelen,filtord)
...
parfor ii = 1:length(trials)
    [hilbert_angles(ii), inst_freq(ii), filt_inst_freq(ii), instchange(ii)] = do_hilbert(trials(ii),filttype,framelen,filtord); 
end 

% mean over trials 
for ii = 1:length(filt_inst_freq)
    aMat = cell2matnan(filt_inst_freq(ii).trial);
