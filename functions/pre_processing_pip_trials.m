function [trials] = pre_processing_pip_trials(dataset,bpfilt,bpwidth,order)
...
parfor ii = 1:length(dataset)
    ...
    trials(ii) = do_trialselection(dataset(ii).path,dataset(ii).file, chan, dataset(ii).stimno, bpfilt, bpwidth, order);
end 
