...
annotation(f,'textbox',[0.1 0.01 0.8 0.1],'String',sprintf("MC: %s , filttype: %s, bpwidth: %i - %iHz, Smooth filter: %s, Filter Window: %i, Filter Order (sgolay): %i  \nAttend In cor: %.2f, Attend Out cor: %.2f", ...
    num2str(params.MC),params.bptype,params.lower,params.upper,params.filttype,params.framelen,params.filtord,in_corr.sess_m(1,1),out_corr.sess_m(1,1)))
...
for ii = 1:length(in_corr.sess)
    ...
    title(sprintf("V1a & V4 inst. freq detrended, pair %i. r = %.2f, p = %.2f",ii,in_corr.sess(1,ii),in_corr.sess(2,ii)))
    legend({'AttendIn','V4'})
    xlabel('Time [ms]')
    ylabel('Frequency [Hz]')
    xlim([0 2000]) 
    ...
    title(sprintf("V1n & V4 inst. freq detrended, pair %i. r = %.2f, p = %.2f",ii,out_corr.sess(1,ii),out_corr.sess(2,ii)))
    legend({'AttendOut','V4'})
    xlabel('Time [ms]')
    ylabel('Frequency [Hz]')
    xlim([0 2000])    
...
yl = yline([0],'--','color',[0.7 0.7 0.7]);
annotation(f,'textbox',[0.1 0.01 0.8 0.1],'String',sprintf("MC: %s , filttype: %s, bpwidth: %i - %iHz, Smooth filter: %s, Filter Window: %i, Filter Order (sgolay): %i  \nAttend In cor: %.2f, Attend Out cor: %.2f", ...
    num2str(params.MC),params.bptype,params.lower,params.upper,params.filttype,params.framelen,params.filtord,in_corr_det.sess_m(1,1),out_corr_det.sess_m(1,1)))
title('In vs Out V1/V4 detrended correlations')
title('In vs Out V1/V4 detrended correlations')
ylabel('V1/V4 AttendIn')
xlabel('V1/V4 AttendOut')
saveas(gcf,fullfile(foldername,sprintf("In_vs_out_Scatter_detrended.fig")))
saveas(gcf,fullfile(foldername,sprintf("In_vs_out_Scatter_detrended.jpg")))
